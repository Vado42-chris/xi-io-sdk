...
function currentSeam(family, extra = {}) {
  ...
  if (peerFamilies.has(family)) Object.assign(row, {
    ...
    binding_root_ref: 'root:test', source_digest_ref: `sha256:${'a'.repeat(64)}`,
...
function allCurrent() { return STANDARD_REJOIN_FAMILIES.map((family) => currentSeam(family)); }
function compile(seams, rootExtra = {}) {
  return compileRejoinSeams({ root_ref: 'root:test', agent_ref: 'agent:test', subject_generation: 'g2', current_generation: 'g2', seams, ...rootExtra });
}
...
providerWait[mcpIndex] = currentSeam('MCP', { state: 'UNKNOWN', evidence_ref: null, readback_ref: null, resolution_class: 'TRUE_WAIT', wake_when: 'provider://mcp/registration-readback' });
...
malformedWait[mcpIndex] = currentSeam('MCP', { state: 'UNKNOWN', evidence_ref: null, readback_ref: null, resolution_class: 'TRUE_WAIT', wake_when: null });
result = compile(malformedWait);
assert.equal(result.machine_resolvable_refresh_count, 1);
assert.ok(result.seams[mcpIndex].invalidators.includes('TRUE_WAIT_WITHOUT_WAKE'));

const peerIdentityMissing = allCurrent();
peerIdentityMissing[mcpIndex] = currentSeam('MCP', { principal_ref: null, assignment_ref: null, assignment_receipt_ref: null, authority_receipt_ref: null });
result = compile(peerIdentityMissing);
assert.equal(result.status, 'UNKNOWN');
assert.equal(result.seams[mcpIndex].current, false);

const guessedFunctionCall = allCurrent();
guessedFunctionCall[mcpIndex] = currentSeam('MCP', { tool_binding_ref: null, operation_profile_ref: null, resource_ref: null, path_binding_ref: null });
result = compile(guessedFunctionCall);
assert.equal(result.status, 'UNKNOWN');
assert.ok(result.seams[mcpIndex].missing_bindings.includes('tool_binding_ref'));
assert.ok(result.seams[mcpIndex].missing_bindings.includes('path_or_storage_binding_ref'));

const noToolBinding = allCurrent();
noToolBinding[mcpIndex] = currentSeam('MCP', { tool_binding_ref: null });
result = compile(noToolBinding);
assert.equal(result.status, 'UNKNOWN');
assert.ok(result.seams[mcpIndex].missing_bindings.includes('tool_binding_ref'));

const wrongOperation = allCurrent();
wrongOperation[mcpIndex] = currentSeam('MCP', {
  requested_operation_class: 'WRITE', bound_operation_class: 'READ', expected_effect_class: 'LOCAL_WRITE', observed_effect_class: 'LOCAL_WRITE',
});
result = compile(wrongOperation);
assert.equal(result.status, 'UNKNOWN');
assert.ok(result.seams[mcpIndex].invalidators.includes('OPERATION_CLASS_MISMATCH'));

const localWriteForProvider = allCurrent();
localWriteForProvider[mcpIndex] = currentSeam('MCP', {
  requested_operation_class: 'WRITE', bound_operation_class: 'WRITE', expected_effect_class: 'PROVIDER_WRITE', observed_effect_class: 'LOCAL_WRITE',
  effect_receipt_ref: 'effect:local:g2', provider_occurrence_ref: null,
});
result = compile(localWriteForProvider);
assert.equal(result.status, 'UNKNOWN');
assert.ok(result.seams[mcpIndex].invalidators.includes('EFFECT_CLASS_MISMATCH'));

const fakeC010Status = allCurrent();
fakeC010Status[mcpIndex] = currentSeam('MCP', {
  requested_operation_class: 'WRITE', bound_operation_class: 'WRITE', expected_effect_class: 'PROVIDER_WRITE', observed_effect_class: 'PROVIDER_WRITE',
  effect_receipt_ref: null, provider_occurrence_ref: null,
});
result = compile(fakeC010Status);
assert.equal(result.status, 'UNKNOWN');
assert.ok(result.seams[mcpIndex].missing_bindings.includes('effect_receipt_ref'));
assert.ok(result.seams[mcpIndex].missing_bindings.includes('provider_occurrence_ref'));

const wrongRoot = allCurrent();
wrongRoot[mcpIndex] = currentSeam('MCP', { binding_root_ref: 'root:other' });
result = compile(wrongRoot);
assert.equal(result.status, 'UNKNOWN');
assert.ok(result.seams[mcpIndex].invalidators.includes('BINDING_ROOT_MISMATCH'));

const boundProviderEffect = allCurrent();
boundProviderEffect[mcpIndex] = currentSeam('MCP', {
  requested_operation_class: 'WRITE', bound_operation_class: 'WRITE', expected_effect_class: 'PROVIDER_WRITE', observed_effect_class: 'PROVIDER_WRITE',
  effect_receipt_ref: 'effect:provider:g2', provider_occurrence_ref: 'provider:occurrence:g2',
});
result = compile(boundProviderEffect);
assert.equal(result.status, 'CURRENT_BOUNDED');
assert.equal(result.seams[mcpIndex].current, true);
assert.equal(result.authority_granted, false);
assert.equal(result.provider_effects, 0);
...
crmIdentityMissing[crmIndex] = currentSeam('CRM_MAIL', { principal_ref: null, mailbox_address: null, authority_receipt_ref: null, provider_occurrence_ref: null });
result = compile(crmIdentityMissing);
assert.equal(result.status, 'UNKNOWN');
assert.ok(result.seams[crmIndex].missing_bindings.includes('provider_occurrence_ref'));

const cfIndex = STANDARD_REJOIN_FAMILIES.indexOf('CLOUDFLARE');
const cloudflareHostnameOnly = allCurrent();
cloudflareHostnameOnly[cfIndex] = currentSeam('CLOUDFLARE', { readback_ref: null, provider_occurrence_ref: null });
result = compile(cloudflareHostnameOnly);
assert.equal(result.status, 'UNKNOWN');
assert.ok(result.seams[cfIndex].missing_bindings.includes('provider_occurrence_ref'));
...
console.log('REJOIN_SEAMS_PASS current=17/17 call_binding_required=1 guessed_path_rejected=1 operation_scope_bound=1 local_effect_not_provider=1 fake_status_not_effect=1 root_binding_required=1 provider_occurrence_required=1 peer_identity_access_required=1 crm_mail_provider_occurrence_required=1 cloudflare_provider_occurrence_required=1 stale_only_affected=1 provider_wait=1 return_apply_required=1 detonator_trip_debt_required=1');
