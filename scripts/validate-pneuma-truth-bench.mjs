...
await candidate('GEMINI_SANDBOX_LOCAL_SOCKET_THEATER',{}, {state:'TRUE_WAIT',first_red:'SYNTHETIC_STATE_ROOT_NOT_PHYSICAL_READBACK'});
const sandboxTheater=results.filter(x=>x.id==='GEMINI_SANDBOX_LOCAL_SOCKET_THEATER');
assert.equal(sandboxTheater.length,1);
assert.equal(sandboxTheater[0].proof_scope,'SYNTHETIC_FIXTURE');
assert.equal(sandboxTheater[0].false_green,false);
