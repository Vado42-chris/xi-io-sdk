...
const q = (id, parent=null, overrides={}) => ({
  ...
  template_found_bit:0, primitive_found_bit:0, new_class_bit:0, user_choice_bit:0,
...
{
  const x=base(); Object.assign(x.questions.find(n=>n.question_id==='UNDERSTAND'),{observed_known_bit:0,observed_value_bit:0,evidence_refs:[]});
...
{
  const x=base(); x.questions.find(n=>n.question_id==='ACT').observed_value_bit=0;
  const out=compileUserExperienceSocratic(x);
  assert.equal(out.root_state,'FAIL'); assert.equal(out.next.action,'HOT_PATCH_OR_COLLIDE_EXISTING_OWNER'); checks+=2;
}
{
  const x=base(); Object.assign(x.questions.find(n=>n.question_id==='ACT'),{observed_value_bit:0,template_found_bit:1});
  assert.equal(compileUserExperienceSocratic(x).next.action,'ADOPT_VERIFY_EXISTING'); checks++;
}
{
  const x=base(); Object.assign(x.questions.find(n=>n.question_id==='ACT'),{observed_value_bit:0,new_class_bit:1});
  assert.equal(compileUserExperienceSocratic(x).next.action,'HOT_PATCH_NEW_CLASS'); checks++;
}
{
  const x=base(); Object.assign(x.questions.find(n=>n.question_id==='ACT'),{observed_known_bit:0,observed_value_bit:0,evidence_refs:[],user_choice_bit:1});
  assert.equal(compileUserExperienceSocratic(x).next.action,'USER_CHOICE_REQUIRED'); checks++;
}
{
  const x=base(); x.questions.find(n=>n.question_id==='ACT').evidence_refs=[];
  const out=compileUserExperienceSocratic(x);
...
{
  const x=base(); x.questions=[g('ROOT'),g('ACTION','ROOT'),q('CLICK','ACTION',{observed_value_bit:0}),q('RECOVER','ACTION')];
  const out=compileUserExperienceSocratic(x);
...
{
  const a=base(); Object.assign(a.questions.find(n=>n.question_id==='ACT'),{observed_value_bit:0});
...
}
{
  const x={root_ref:'root:x',user_ref:'user:x',experience_ref:'exp:x',source_generation:'g1',questions:[
    g('ROOT'), g('FLOW','ROOT'), q('DEEP','FLOW',{observed_value_bit:0}), q('SHALLOW','ROOT',{observed_known_bit:0,observed_value_bit:0,evidence_refs:[]})
  ]};
  const out=compileUserExperienceSocratic(x);
...
}
{
  ...
  const out=compileUserExperienceSocratic(x);
  assert.equal(out.first_red.question_id,'ACT'); assert.equal(out.counts.unknown_questions,1); assert.equal(out.counts.mismatch_questions,1); checks+=3;
}

console.log(JSON.stringify({schema:'xiio.sdk.user-experience-socratic-hostiles/v1',result:'PASS',hostiles:24,checks,provider_effects:0,authority_granted:false}));
