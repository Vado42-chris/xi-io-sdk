...
try{
  ...
  assert.equal(r.body?.schema,'xiio.cli.status/v1');
  assert.equal(r.body?.command,'status');

  r=run(['gates','--check','--json']);
  assert.ok([0,1,2].includes(r.status));
  assert.equal(r.body?.schema,'xiio.cli.gates-check/v1');
  assert.equal(r.body?.command,'gates.check');
  assert.equal(r.body?.silent_remainder,0);
  ...
  assert.equal(r.body?.state,'PASS');
  ...
  assert.equal(r.body?.first_red,'SCHEMA_DECLARED');
  ...
  assert.equal(r.body?.first_red,'SILENT_REMAINDER');

  r=run(['verify','--stdin','--json'],{input:'{bad json'});
  assert.equal(r.status,2);
  assert.equal(r.body?.schema,'xiio.cli.verify/v1');
  ...
  assert.equal(r.status,0);
  assert.equal(r.body?.source_ref,path.resolve(file));
