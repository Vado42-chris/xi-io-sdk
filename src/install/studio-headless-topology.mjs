const ROLES=['product_runtime','internal_mail','calendar_crm','tasks_work','switchboard_control'];
const SAFE=/^[a-z0-9][a-z0-9-]{1,62}$/;
const OWNERS={
 product_runtime:{owner_product:'INSTALL_TARGET',host_shell:'xi-io-Studio/Tauri',adapter_refs:[]},
 internal_mail:{owner_product:'xi-io-Inbox',host_shell:'xi-io-Studio/Tauri',adapter_refs:['server/truth-sdk-crm-mailbox.mjs']},
 calendar_crm:{owner_product:'xi-io-Calendar',host_shell:'xi-io-Studio/Tauri',adapter_refs:[]},
 tasks_work:{owner_product:'xi-io-Tasks',host_shell:'xi-io-Studio/Tauri',adapter_refs:['bugzilla','dotproject']},
...
export function compileStudioHeadlessTopology(input){
 ...
 const binsReady=servers.every(x=>x.bin.ref&&x.bin.verified&&x.bin.receipt_ref),runtime=servers[0],mail=servers[1],calendar=servers[2],tasks=servers[3],control=servers[4];
 const cards=[
  ...
  {card_id:'HVT-002',target:'INTERNAL_MAIL_SERVER',owner_product:'xi-io-Inbox',depends_on:['HVT-001'],state:mail.state,why:'Inbox owns communication transport, not CRM or Tasks semantic ownership'},
  {card_id:'HVT-003',target:'CALENDAR_CRM_SERVER',owner_product:'xi-io-Calendar',depends_on:['HVT-001','HVT-002'],state:calendar.state,why:'Calendar owns CRM/chronal relationship context'},
  {card_id:'HVT-004',target:'TASKS_COMPOSITE_SERVER',owner_product:'xi-io-Tasks',depends_on:['HVT-001','HVT-002','HVT-003'],state:tasks.state,why:'Tasks composes Bugzilla + dotProject locally in Tauri and may project to cloud only after qualification'},
  {card_id:'HVT-005',target:'SWITCHBOARD_HEADLESS_CONTROL_SERVER',owner_product:'xi-io-Switchboard',depends_on:['HVT-001','HVT-002','HVT-003','HVT-004'],state:control.state,why:'Switchboard routing follows current Mail, Calendar CRM and Tasks projections'},
  {card_id:'HVT-006',target:'PRODUCT_RUNTIME_SERVER',owner_product:product_id,depends_on:['HVT-001','HVT-002','HVT-003','HVT-004','HVT-005'],state:runtime.state,why:'A product runtime alone cannot participate in governed Studio work'},
  {card_id:'HVT-007',target:'CLOUDFLARE_DEV_PROJECTION',owner_product:'xi-io-Switchboard',depends_on:['HVT-001','HVT-002','HVT-003','HVT-004','HVT-005','HVT-006'],state:'PROPOSED',why:'Cloud projection follows local health and Switchboard admission'},
  {card_id:'HVT-008',target:'HEX_TAURI_INSTALL_RECEIPT',owner_product:'xi-io-HEX',host_shell:'xi-io-Studio/Tauri',depends_on:['HVT-001','HVT-002','HVT-003','HVT-004','HVT-005','HVT-006','HVT-007'],state:'BLOCKED',why:'Installation closes only after native and cloud-projection readback'}
 ];
 ...
 return {schema:'xiio.sdk.studio-headless-topology/v1',product_id,minimum_server_count:5,servers,bin_registry:{owner_product:'xi-io_bins',global_path_mutation:false,all_roles_receipted:binsReady},quarantine:{loopback_only:true,state_root:root,user_existing_settings_mutated:false,separate_ports:true,dev_namespace_only:true},cloudflare:{hostname:`${product_id}.dev.xi-io.net`,state:'PROPOSED_NOT_APPLIED',owner_product:'xi-io-Switchboard',ward_admission_required:true},mail:{owner_product:'xi-io-Inbox',transport:'internal_mail',external_email_fallback:false},calendar_crm:{owner_product:'xi-io-Calendar',transport:'calendar_crm'},tasks:{owner_product:'xi-io-Tasks',composition:['bugzilla','dotproject'],local_runtime:'TAURI_LOCAL_FIRST',cloud_projection:'QUALIFIED_ONLY'},installer:{owner_product:'xi-io-HEX',host_shell:'xi-io-Studio/Tauri'},hvt_punchcards:cards,next_hvt:first?.card_id||null,closure:false,authority:{cloudflare:false,service_install:false,provider_effect:false},hard:['ONE_SERVER!=STUDIO_INSTALL','INBOX!=CRM','CRM=CALENDAR','TASKS=BUGZILLA+DOTPROJECT_COMPOSITE','BUGZILLA!=TASKS_PRODUCT','DOTPROJECT!=TASKS_PRODUCT','BIN_NAME!=EXECUTABLE_RECEIPT','ROSTER_SAMPLE!=ROSTER','CRM_WITHOUT_INTERNAL_MAIL=UNUSABLE','CADENCE_WITHOUT_SWITCHBOARD=PROJECTION_ONLY','LOCAL_HEALTH!=DEV_LIVE','PROPOSED_HOSTNAME!=DNS_EFFECT','USER_SETTINGS_MUTATION=DENIED','EXISTING_OWNER!=DUPLICATE_IMPLEMENTATION']};
}
