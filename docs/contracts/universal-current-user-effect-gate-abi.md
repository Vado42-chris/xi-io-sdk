...
Status: accepted-source SDK ABI on current main; runtime/readback remains separately qualified.
...
CONSEQUENTIAL EFFECT = WAIT until an explicit CURRENT user instruction
is bound to the exact occurrence and exact requested effect.
```

Prior approval does not persist or replay.

A current user instruction is a required gate only. It does not mint effect authority. Ward, Switchboard, qualification, admission, provider policy, product policy, and domain-specific legal/security gates remain separate.
...
ROTFL_TEMPLATE_RUNTIME_PASS != EFFECT_AUTHORITY
```
...
2. prior/different instruction -> WAIT/fail validation.
3. same instruction but different effect ref -> WAIT/fail validation.
4. missing occurrence ref -> WAIT/fail validation.
5. missing requested effect ref -> WAIT/fail validation.
...
9. ACK terminal state without completed ROTFL template runtime -> FAIL independently of effect policy.
10. valid ROTFL template runtime without valid effect policy -> FAIL independently of template runtime.
...
```text
src/acks/distributed.mjs#validateUniversalEffectPolicy
src/acks/distributed.mjs#validateDistributedAck
src/acks/distributed.mjs#validateRotflDistributedAck
src/mail/internal-agent-envelope.mjs
scripts/validate-rotfl-ack.mjs
scripts/validate-baseline-cli.mjs
scripts/validate-internal-agent-envelope.mjs
```

This ABI is composed with the current ACK ROTFL template runtime. It does not replace it.
